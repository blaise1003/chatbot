...
if (!function_exists('admin_apply_security_headers')) {
    function admin_apply_security_headers(): void
    {
        if (headers_sent()) {
            return;
        }
        ...
        header("Content-Security-Policy: default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'");
    }
}

admin_apply_security_headers();
